<template>
	<view>
		<view
			class="header"
			:class="{ fixed: navFixed, absolute: type == 'transparent', line: navLine }"
			:style="{ height: 44 + statusBarHeight + 'px', background: bgColor, color: fontColor, opacity: transparentValue }"
		>
			<view>
				...
				<view class="title-switch" v-if="titleswitch">
					<view class="title-switch-box" v-for="(item, index) in titles" :key="index" :class="{ active: item.idx == isactive }" @tap="goaddres(item.idx)">
						<text class="title-switch-text">{{ item.title }}</text>
						<view class="title-switch-line" v-if="item.idx == isactive"></view>
					</view>
				</view>
			</view>
			<!-- #ifdef APP-PLUS||H5 -->
		...
		<view
			...
			:style="{ height: 44 + statusBarHeight + 'px', color: transparentFixedFontColor, opacity: 1 - transparentValue, zIndex: transparentValue < 0.3 ? 100 : 90 }"
		>
			<view>
				<view class="left_info" v-if="back || $slots.left || (firstPage && back)">
...
const mainPagePath = ['pages/home/home', '/pages/partTime/partTime', '/pages/myPartTime/myPartTime', '/pages/my/my','pages/homePageSub/reserve/reserve'];
//返回首页的地址
...
.header {
	...
	.left_info {
		...
		margin-left: 16upx;
		.back {
			background-image: url('https://pic.cwyyt.cn/upload/yyticons/103806386_back.png');
		...
		.home {
			background-image: url('https://pic.cwyyt.cn/upload/yyticons/104402442_home.png');
			background-position: center center;
			background-repeat: no-repeat;
			background-size: auto 60%;
			width: $height;

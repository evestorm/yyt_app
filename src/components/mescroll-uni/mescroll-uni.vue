<template>
	<view class="mescroll-uni-warp">
		...
		>
			<!-- 添加relative，解决iOS下下拉刷新后再次上拉刷新导致页面显示不全问题 -->
			<!-- <view :style="{ transform: translateY, transition: transition }"> -->
			<view :style="{ transform: translateY, transition: transition, position: 'relative' }">
				<!-- 下拉加载区域-->
				...
				<view v-if="right.use" class="right-swiper-content">{{ right.content }}</view>
				<view v-if="left.use" class="left-swiper-content">{{ left.content }}</view>
				<!-- 列表内容 -->
				<slot></slot>
	...
	created() {
		...
		let diyOption = {
			...
			left: {
				// 左滑动
				callback: function(mescroll) {
					vm.$emit('left', mescroll);
				}
			},
			right: {
				// 右滑动事件
				callback: function(mescroll) {
					vm.$emit('right', mescroll);
				}
			}
		};

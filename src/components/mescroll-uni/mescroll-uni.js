...
MeScroll.prototype.touchendEvent = function(e) {
	if (!this.optDown.use) return;
	const subX=e.changedTouches[0].clientX-this.startPoint.x;
	const subY=e.changedTouches[0].clientY - this.startPoint.y;
	if(subY>50 || subY<-50){
		// 如果下拉区域高度已改变,则需重置回来
		if (this.isMoveDown) {
		...
		if(subX>50){
			// console.log('右滑');
		    this.options.right.callback&&this.options.right.callback(this);
		}else if(subX<-50){
			 this.options.left.callback&&this.options.left.callback(this);
			// console.log('左滑');
		}else{
			// console.log('无效');
		}
